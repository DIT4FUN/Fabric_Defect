...
with fluid.program_guard(main_program=defectProgram, startup_program=startup):
    ...
    def create_loss(predict, label):
        ...
        predict = fluid.layers.reshape(predict, shape=[-1, 183600, class_dim])
        label = fluid.layers.reshape(label, shape=[-1, 183600, 1])
        label = fluid.layers.cast(label, dtype="int64")

        # predict = fluid.layers.gather(predict, mask)  # 清空多余项
    ...
    label_sub = fluid.layers.data(name='label_sub1', shape=[1, 300, 612], dtype='int32')
    ...
    # 定义损失函数
    cost_Base_f = create_loss(net_x, label_sub)
...
prebatch_reader = paddle.batch(
    ...
    batch_size=READIMG)
prefeeder = fluid.DataFeeder(place=place, feed_list=[image, label_sub])
